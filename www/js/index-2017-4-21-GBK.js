...
$(document).on("click",".deleteDevice",function(e,d){
    ...
    try{
        ...
        console.log("deleteDevice error in line:158")
    }
    // 将打开的对话框关闭
    try{
...
$(document).on("click","#saveChangeG",function(event,data){
    ...
    try{
        ...
        console.log("save Group change error")
    }
    //关闭 popup
    $("#groupSet").popup("close");
    //alert("确定要");
...
$(document).on("click","#startScan",function(e,d){
    ...
    if(listShow == "associate"){
        ...
        listShow = "scan";
            ...
            //list.innerHTML +=(ble);
            var deviceFind = document.createElement("tr");
            deviceFind.innerHTML = "<td id="+ ">"+"address :  "+"asdlfk"+"\n设备："+"light"+"</td>";
            deviceFind.onclick=(function(){
                return function(aa){
                    console.log("alksdjalskdf;alskdfalskjfda;lskjdfa");
                    if(aa&&aa.stopPropagation){//非IE浏览器
                        aa.stopPropagation();
                    }else{//IE浏览器
                        window.event.cancelBubble=true;
